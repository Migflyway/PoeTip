...
namespace tools.Dev
{
    public class Api
    {
        ...
        private static bool UpdatingFromJson { get; set; } = false;
        public static bool UpdatingFromAPI { get; set; } = false;

        private static string NinjaDirectory = ".//NinjaData//";
        public static string[] GatherLeagueNames()
        {
            var leagueListFromUrl = ninjaapi.DownloadFromUrl("http://api.pathofexile.com/leagues?type=main&compact=1");
            var leagueData = JsonConvert.DeserializeObject<List<Leagues>>(leagueListFromUrl);
            var leagueList = (from league in leagueData where !league.Id.Contains("SSF") select league.Id).ToArray();
            return leagueList;
        }
    ...
    public class NinjaData
    {
        ...
        public HelmetEnchants.RootObject HelmetEnchants { get; set; }
        ...
        public Resonators.RootObject Resonators { get; set; }
        public Fossils.RootObject Fossils { get; set; }
        public Scarab.RootObject Scarab { get; set; }
    }
